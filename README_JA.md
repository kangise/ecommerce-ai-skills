...
| 運用 UI | `/app` の 7 ビュー：ブリーフィング、agent、証拠、承認、接続、自動化、監査。中英日の設定を保持、ライト/ダークテーマ対応 |

<p align="center">
  <img src="assets/screenshots/approvals-en-dark.png" alt="承認ビュー：操作・リスク・ロールバック計画・有効期限を持つアクション提案フォームと、その下の実行済み人による確認レコード" width="100%">
</p>
<p align="center"><sub>承認：外部への書き込みはすべてロールバック計画と有効期限付きの提案になり、別の権限を持つユーザーが承認してから Operator が実行する。</sub></p>
